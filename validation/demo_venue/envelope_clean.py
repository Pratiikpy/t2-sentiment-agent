...
CRYPTO = ['BTCUSDT']
STOCKS = ['SP500USDT','NDX100USDT','HOODUSDT','COINUSDT','MSTRUSDT','METAUSDT','AMZNUSDT','GOOGLUSDT','AAPLUSDT','CRCLUSDT','NVDAUSDT','TSLAUSDT','SNDKUSDT']
px = {}
for s in CRYPTO:
    px[s] = {int(r[0]): r[4] for r in candles_v2(s, 'USDT-FUTURES', '1H', n_pages=12)}
for s in STOCKS:
    f = os.path.join(WB, s + '.json')
    if os.path.exists(f):
        px[s] = {int(x[0]): float(x[4]) for x in json.load(open(f))}
    else:
        px[s] = {int(r[0]): r[4] for r in candles_v2(s, 'USDT-FUTURES', '1H', n_pages=12)}
...
json.dump(res, open('envelope_clean.json', 'w'), indent=1)
